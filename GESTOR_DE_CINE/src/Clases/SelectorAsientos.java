...
public class SelectorAsientos extends JFrame implements PropertyChangeListener {
    ...
    public SelectorAsientos() {
        ...
        // IMPORTANTE: Primero cargar el estado, luego inicializar componentes
        System.out.println("🚀 Iniciando aplicación...");
        gestorJson.cargarEstadoGuardado();

        inicializarComponentes();

        // Forzar actualización visual después de cargar
    ...
    private void limpiarSelecciones() {
        ...
        if (respuesta == JOptionPane.YES_OPTION) {
            ...
            actualizarVisualizacionAsientos();
            JOptionPane.showMessageDialog(this, "Selecciones limpiadas correctamente");
        }
    }

    private void recargarDesdeJSON() {
        int respuesta = JOptionPane.showConfirmDialog(this,
                "¿Recargar desde el archivo JSON?\n" +
        ...
        if (respuesta == JOptionPane.YES_OPTION) {
            gestorJson.cargarEstadoGuardado();
            actualizarVisualizacionAsientos();
            JOptionPane.showMessageDialog(this, "Datos recargados desde JSON correctamente");
        }
    }
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if ("asientoCambiado".equals(evt.getPropertyName())) {
            ...
            // Guardar estado completo cuando cambia un asiento
            gestorJson.guardarEstadoCompleto();

            // Si el source es un AsientoButton, también podríamos guardar solo ese asiento
            if (evt.getSource() instanceof AsientoButton) {
                AsientoButton asiento = (AsientoButton) evt.getSource();
                gestorJson.guardarEstadoAsiento(asiento.getFila(), asiento.getColumna());
            }
        }
    }
...
//
//        if (respuesta == JOptionPane.YES_OPTION) {
//            // Limpiar selecciones en la sala
//            for (int i = 0; i < FILAS_ASIENTOS; i++) {
//                for (int j = 0; j < COLUMNAS; j++) {
//                    if (sala.getEstadoAsiento(i, j) == SalaCine.EstadoAsiento.SELECCIONADO) {
//                        sala.setEstadoAsiento(i, j, SalaCine.EstadoAsiento.LIBRE);
//                    }
//                }
//            }
//
